...
class FSRGenerator:
    ...
    def generate_fsr(self, faculty_data, research_data, extensions_data, output_path):
        ...
        research_row = self._fill_research(
            ws, research_data, research_start_row)
    ...
    def _fill_extensions(self, ws, extensions_data, start_row):
        ...
        for idx, extension in enumerate(extensions_data, 1):
            ...
            if start_date:
                cell_h.value = start_date if isinstance(
                    start_date, str) else start_date.strftime('%m/%d/%Y')
            cell_h.border = self._create_border()
            ...
            if end_date:
                cell_i.value = end_date if isinstance(
                    end_date, str) else end_date.strftime('%Y-%m-%d %H:%M:%S')
            cell_i.border = self._create_border()
    ...
    def generate_fsr_for_member(self, member_id, semester='2nd Semester', academic_year='2025-2026'):
        ...
        # Fetch research data - use 'uid' not 'member_id'
        research_docs = db.collection('research').where(
            'uid', '==', member_id).stream()
        research_data = [doc.to_dict() for doc in research_docs]

        # Fetch extension data - use 'uid' not 'member_id'
        extension_docs = db.collection('extensions').where(
            'uid', '==', member_id).stream()
